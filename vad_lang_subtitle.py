...
def split_by_spaces_for_non_spaced_languages(segments):
    """
    Split segments by spaces for languages that don't naturally use spaces (Chinese, Japanese, etc.).
    In these languages, spaces often indicate natural pauses.
    
    Parameters:
    - segments: List of subtitle segments
    
    Returns:
    - List of segments split at spaces for appropriate languages
    """
    refined_segments = []
    non_spaced_languages = {'zh', 'ja', 'ko', 'ar', 'yue'}
    
    for segment in segments:
        lang = segment.get('lang', 'en')
        
        if lang not in non_spaced_languages or ' ' not in segment['text']:
            refined_segments.append(segment)
            continue
            
        words = segment.get('words', [])
        if not words:
            # Fallback: split by spaces without word timestamps
            text_parts = segment['text'].split(' ')
            if len(text_parts) > 1:
                # Simple equal time division
                total_duration = segment['end'] - segment['start']
                time_per_part = total_duration // len(text_parts)
                
                for i, part in enumerate(text_parts):
                    if part.strip():
                        refined_segments.append({
                            'start': segment['start'] + i * time_per_part,
                            'end': segment['start'] + (i + 1) * time_per_part,
                            'lang': segment['lang'],
                            'text': part.strip()
                        })
            else:
        ...
        # Split using word-level timestamps, looking for spaces
        ...
        for i, word in enumerate(words):
            ...
            # Check if this word is followed by a space that indicates a natural break
            current_text = ''.join(w['word'] for w in current_words)
            
            # If we find a space after this word and have accumulated enough content
            if (i < len(words) - 1 and len(current_words) >= 2 and 
                any(' ' in w['word'] for w in current_words[-2:])):
...
def get_language_limits(lang):
    """
    Get appropriate character and word limits based on language characteristics.
    
    Parameters:
    - lang: Language code
    
    Returns:
    - Tuple of (max_chars, max_words)
    """
    # Chinese, Japanese, Korean have denser information per character
    cjk_languages = {'zh', 'ja', 'ko', 'yue'}
    # Arabic reads right to left and has different spacing
    rtl_languages = {'ar'}
    
    if lang in cjk_languages:
        return 30, 15  # Fewer characters but more words since no spaces
    elif lang in rtl_languages:
        return 50, 12
    else:
        return 60, 10  # Default for Latin-based languages


def split_long_segments_by_length(segments):
    """
    Split segments that are too long by character count or word count.
    Uses language-appropriate limits.
    ...
    for segment in segments:
        ...
        max_chars, max_words = get_language_limits(lang)
        
        # For CJK languages, count characters; for others, count words primarily
        cjk_languages = {'zh', 'ja', 'ko', 'yue'}
        
        if lang in cjk_languages:
            # For CJK, focus on character count
            is_too_long = len(text) > max_chars
        else:
            # For other languages, focus on word count but also check characters
            word_count = len(text.split()) if text else 0
            is_too_long = word_count > max_words or len(text) > max_chars
        ...
        if words and len(words) > 1:
            # Split using word timestamps - find the best split point
            split_index = len(words) // 2
            
            # For CJK languages, try to find a better split point
            if lang in cjk_languages:
                target_chars = len(text) // 2
                char_count = 0
                for i, word in enumerate(words):
                    char_count += len(word['word'])
                    if char_count >= target_chars and i > 0:
                        split_index = i
                        break
            ...
            refined_segments.extend(split_long_segments_by_length([first_segment]))
            refined_segments.extend(split_long_segments_by_length([second_segment]))
        else:
            # Fallback: split by character/word count without word timestamps
            if lang in cjk_languages:
                # Split by characters for CJK
                mid_char = len(text) // 2
                ...
                # Split by words for other languages
                text_words = text.split()
                mid_word = len(text_words) // 2
                
                first_text = ' '.join(text_words[:mid_word])
                second_text = ' '.join(text_words[mid_word:])
...
def refine_with_vad_pauses(segments, vad_segments, sample_rate, max_duration=6.0):
    """
    Use VAD segments to find finer natural breaks within long segments.
    
    Parameters:
    - segments: List of subtitle segments
    - vad_segments: VAD segments with speech/silence boundaries
    - sample_rate: Audio sample rate
    - max_duration: Only split segments longer than this duration
    
    Returns:
    - List of segments refined using VAD boundaries
    """
    refined_segments = []
    ...
    for segment in segments:
        ...
        # Find VAD boundaries within this segment
        segment_vad = []
        for vad in vad_segments:
            if (vad['start'] >= segment['start'] and vad['end'] <= segment['end']) or \
               (vad['start'] < segment['end'] and vad['end'] > segment['start']):
                segment_vad.append(vad)
        
        if len(segment_vad) <= 1:
            refined_segments.append(segment)
            continue
        
        # Split the segment at VAD boundaries
        ...
        for word in words:
            ...
            # Check if this word is near a VAD boundary (within 100ms)
            word_end = word['end']
            for vad in segment_vad:
                if abs(word_end - vad['end']) < (0.1 * sample_rate) and len(current_words) >= 3:
                    # Create a segment up to this point
                    refined_segments.append({
                        'start': current_start,
                        'end': word_end,
                        'lang': segment['lang'],
...
def enhanced_punctuation_split(segments):
    ...
    pause_words = {
        'en': [
            ...
            ' and ', ' but ', ' or ', ' nor ', ' for ', ' so ', ' yet ', ' while ', ' since ', ' until ', ' because ', ' although ', ' though ', ' unless ', ' if ', ' when ', ' where ', ' why ', ' how ', ' that ', ' which ', ' who ',
            # Transitional words and phrases
            ' however ', ' therefore ', ' moreover ', ' furthermore ', ' nevertheless ', ' nonetheless ', ' meanwhile ', ' consequently ', ' subsequently ', ' additionally ', ' similarly ', ' likewise ', ' conversely ', ' alternatively ', ' instead ', ' rather ', ' otherwise ', ' hence ', ' thus ', ' accordingly ', ' besides ', ' also ', ' too ', ' in addition ', ' on the other hand ', ' in contrast ', ' as a result ', ' for example ', ' for instance ', ' in particular ', ' specifically ', ' generally ', ' typically ', ' usually ', ' often ', ' sometimes ', ' occasionally ', ' frequently ', ' indeed ', ' actually ', ' really ', ' certainly ', ' obviously ', ' clearly ', ' apparently ', ' presumably ', ' unfortunately ', ' fortunately ', ' surprisingly ', ' interestingly ', ' importantly ', ' significantly ', ' notably ', ' especially ', ' particularly ', ' mainly ', ' primarily ', ' essentially ', ' basically ', ' simply ', ' merely ', ' only ', ' just ', ' even ', ' still ', ' already ', ' now ', ' then ', ' next ', ' later ', ' earlier ', ' before ', ' after ', ' during ', ' first ', ' second ', ' third ', ' finally ', ' lastly ', ' in conclusion ', ' to summarize ', ' in summary '
        ],
        'zh': [
            # 转折连词
            '但是', '可是', '不过', '然而', '却', '而', '只是', '只不过', '虽然', '尽管', '即使', '哪怕', '就算',
            # 因果关系
            '因为', '由于', '所以', '因此', '因而', '故而', '于是', '结果', '导致', '造成', '引起',
            # 递进关系
            '而且', '并且', '还有', '另外', '此外', '再者', '更重要的是', '不仅', '不但', '同时', '与此同时',
            # 时间顺序
            '然后', '接着', '随后', '之后', '后来', '最后', '最终', '开始', '首先', '其次', '再次', '接下来', '紧接着', '当时', '那时', '现在', '目前', '将来', '以后', '以前', '刚才', '马上', '立即',
            # 举例说明
            '比如', '例如', '比方说', '像是', '譬如', '好比', '如同', '正如', '就像',
            # 总结概括
            '总之', '总的来说', '综上所述', '简而言之', '换句话说', '也就是说', '总而言之',
            # 条件假设
            '如果', '假如', '要是', '倘若', '万一', '除非', '只要', '只有', '一旦',
            # 并列关系
            '一边', '一方面', '另一方面', '既', '也', '又', '还',
            # 强调突出
            '特别是', '尤其是', '主要是', '关键是', '重要的是', '值得注意的是',
            # 语气词和停顿词
            '嗯', '啊', '呃', '额', '嗯嗯', '对', '是', '对的', '没错', '当然', '肯定', '绝对', '确实', '真的', '实际上', '其实', '说实话', '老实说'
        ],
        'ja': [
            # 基本的な接続詞
            'そして', 'でも', 'しかし', 'だから', 'それで', 'それに', 'また', 'さらに', 'ところで', 'ただし', 'なので', 'けれど', 'けれども', 'だが', 'しかも', 'そこで', 'すると', 'それなら', 'それでは', 'ところが', 'なお', 'あるいは', 'または', 'もしくは', 'つまり', 'すなわち',
            # 時間関係
            'そのとき', 'その後', 'それから', '次に', '最初に', '最後に', '今度は', '今回は', '前回は', 'まず', 'はじめに', '終わりに', '今では', '現在は', 'これまで', 'これから', 'いつも', '時々', 'たまに', 'よく',
            # 理由・原因
            'なぜなら', 'というのは', 'そのため', 'したがって', 'ゆえに', 'だからこそ', 'おかげで', 'せいで',
            # 例示・説明
            '例えば', 'たとえば', '具体的には', 'つまり', 'すなわち', '要するに', '簡単に言えば', '言い換えれば',
            # 逆接・対比
            '逆に', '反対に', '一方で', '他方で', 'むしろ', 'かえって', 'それに対して', 'これに対して',
            # 追加・補足
            'それに加えて', 'その上', 'さらに言えば', '加えて', 'ちなみに', 'なお', 'また',
            # 結論・まとめ
            '結論として', '要するに', '簡単に言えば', 'とにかく', '結局', '最終的に', '全体的に'
        ],
        'ar': [
            # Arabic conjunctions and connectors
            'و', 'لكن', 'أو', 'إذا', 'عندما', 'حيث', 'لأن', 'كما', 'بينما', 'حتى', 'منذ', 'بعد', 'قبل', 'أثناء', 'رغم', 'مع ذلك', 'لذلك', 'إذن', 'أيضا', 'كذلك', 'مثل', 'بالإضافة إلى', 'من ناحية أخرى', 'في المقابل', 'على سبيل المثال', 'أخيرا', 'أولا', 'ثانيا', 'ثالثا'
        ],
        'ko': [
            # Korean connectors and particles
            '그리고', '그러나', '하지만', '그래서', '따라서', '또한', '또', '그런데', '그럼', '그렇다면', '왜냐하면', '예를 들어', '즉', '다시 말해서', '한편', '반면에', '그러므로', '그럼에도 불구하고', '게다가', '더욱이', '특히', '먼저', '다음에', '마지막으로', '결국', '결론적으로', '요약하면', '일반적으로', '보통', '때때로', '항상', '이제', '이미', '아직', '곧', '나중에', '먼저'
        ],
        'vi': [
            # Vietnamese connectors
            'và', 'nhưng', 'hoặc', 'nếu', 'khi', 'nơi', 'tại sao', 'vì', 'mặc dù', 'trong khi', 'cho đến khi', 'từ khi', 'sau khi', 'trước khi', 'trong khi', 'tuy nhiên', 'do đó', 'vì vậy', 'cũng', 'như', 'thêm vào đó', 'mặt khác', 'ngược lại', 'ví dụ', 'cuối cùng', 'đầu tiên', 'thứ hai', 'thứ ba'
        ],
        'es': [
            # Spanish connectors
            ' y ', ' pero ', ' o ', ' si ', ' cuando ', ' donde ', ' porque ', ' aunque ', ' mientras ', ' hasta ', ' desde ', ' después ', ' antes ', ' durante ', ' sin embargo ', ' por lo tanto ', ' también ', ' como ', ' además ', ' por otro lado ', ' en cambio ', ' por ejemplo ', ' finalmente ', ' primero ', ' segundo ', ' tercero ', ' entonces ', ' así que ', ' pues ', ' luego ', ' después ', ' ahora ', ' ya ', ' todavía ', ' pronto ', ' más tarde ', ' antes '
        ],
        'fr': [
            # French connectors
            ' et ', ' mais ', ' ou ', ' si ', ' quand ', ' où ', ' parce que ', ' bien que ', ' pendant que ', ' jusqu\'à ', ' depuis ', ' après ', ' avant ', ' pendant ', ' cependant ', ' donc ', ' aussi ', ' comme ', ' en plus ', ' d\'autre part ', ' en revanche ', ' par exemple ', ' enfin ', ' d\'abord ', ' ensuite ', ' puis ', ' alors ', ' maintenant ', ' déjà ', ' encore ', ' bientôt ', ' plus tard ', ' avant '
        ]
    }
    ...
    for segment in segments:
        ...
        for i, word in enumerate(words):
            ...
            # Check for pause words specific to the language
            has_pause_word = False
            current_text = ''.join(w['word'] for w in current_words)
            if lang in pause_words:
                for pause in pause_words[lang]:
                    if pause in current_text.lower():
                        has_pause_word = True
                        break
            
            # For Chinese, also look for natural repetition patterns that indicate pauses
            has_repetition = False
            if lang in {'zh', 'yue'} and len(current_words) >= 4:
                # Look for repeated characters or phrases
                text = current_text.replace(' ', '')
                for j in range(2, min(6, len(text)//2 + 1)):
                    for k in range(len(text) - 2*j + 1):
                        if text[k:k+j] == text[k+j:k+2*j]:
                            has_repetition = True
                            break
                    if has_repetition:
                        break
            
            # Split if conditions are met and we have minimum content
            min_words = 2 if lang in {'zh', 'ja', 'ko', 'yue'} else 3
            should_split = (has_punctuation or has_pause_word or has_repetition) and len(current_words) >= min_words
...
def reduce_subtitle_length(segments, sample_rate=16000, vad_segments=None):
    """
    Main function to reduce subtitle length using multiple sophisticated methods.
    
    Parameters:
    - segments: List of subtitle segments
    - sample_rate: Audio sample rate
    - vad_segments: VAD segments for finding natural pauses
    
    Returns:
    - List of shortened segments
    """
    print("Reducing subtitle length with multi-layered approach...")
    
    # Step 1: Use VAD to find finer natural breaks within long segments
    if vad_segments:
        segments = refine_with_vad_pauses(segments, vad_segments, sample_rate)
    
    # Step 2: Split by spaces for languages that don't naturally use spaces
    segments = split_by_spaces_for_non_spaced_languages(segments)
    
    # Step 3: Enhanced punctuation splitting with pause words
    segments = enhanced_punctuation_split(segments)
    
    # Step 4: Final length-based splitting with language-appropriate limits
    segments = split_long_segments_by_length(segments)
...
if __name__ == "__main__": 
    ...
    if not (os.path.exists(srt_path) or os.path.exists(json_path)) or args.force:
        ...
        try:
            ...
            # Reduce subtitle line length - NEW ADDITION with VAD integration
            final_subtitles = reduce_subtitle_length(
                final_subtitles, 
                sample_rate=sampling_rate,
                vad_segments=speech_timestamps
            )
